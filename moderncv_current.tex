...
\usepackage[backend=biber, style=authoryear-comp, sorting=ydnt, maxbibnames=99]{biblatex}
...
\usepackage{etoolbox} % required for list control

% Custom numbered bibliography with reverse order and left-aligned entries
\defbibenvironment{bibliography}
  {\begin{enumerate}
    \setcounter{enumi}{\value{bibcount}} % start counting from total number
    \renewcommand*{\labelenumi}{[\theenumi]}
    \setlength{\leftmargin}{2.5em} % aligns with year margin
...
% Counter to reverse item numbering
\newcounter{bibcount}
...
\begin{refsection}[articles_and_chapters.bib]
\nocite{*}
\defbibfilter{all}{}
\setcounter{bibcount}{\bibfilteredcount{all}}
\printbibliography[heading=none, filter=all]\end{refsection}
